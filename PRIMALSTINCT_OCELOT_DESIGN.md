...
| 过热检查周期 | 无 | 无 | 无 | 20 秒 | 10 秒 |
| 过热持续时间 | 无 | 无 | 无 | 5 秒 | 20 秒 |
| 图鉴 / 告示牌乱码比例 | 0 | 0 | 15% | 35% | 100% |

过热检查 8 格内代理原生可攻击对象，50% 概率施加 Buff；Buff 存在时不刷新，没有额外冷却。创造模式不接管。满值锁定不再阻止 AI；锁定演出期间暂缓接管。
...
L2 起，16 格内符合代理攻击类型的实体显示红色穿墙轮廓。轮廓检测忽略视线；过热检测保持原生视线条件。此检测表示“附近可攻击对象”，并不要求代理已经选中或追击目标。
